...
def create_detailed_report_pdf(user, results):
    ...
    # Diet & Exercise (always include — it's fine for healthy too)
    pdf.setFont("Helvetica-Bold", 14)
    ...
    pdf.drawString(60, 100, "Generated by Pancreatic Cancer Detection System – City Engineering College")
    ...
    # footer on page 2
    pdf.setFont("Helvetica-Oblique", 10)
    pdf.drawString(60, 85, "Generated by Pancreatic Cancer Detection System – City Engineering College")
...
class Doctor(db.Model):
    ...
    password = db.Column(db.String(200), nullable=False)
    specialization = db.Column(db.String(120), nullable=False)   # ✅ ADD THIS
...
def view_users():
    ...
    if session.get('user_name') != "Vishnu B":
        flash("🚫 Access denied. Only admin can view user data.", "danger")
...
app.config['MAIL_USERNAME'] = 'yourmedicalreport@gmail.com'         # 🔹 Replace with your email
app.config['MAIL_PASSWORD'] = 'fwzbyfbushrjbfma'       # 🔹 Use App Password (not your Gmail password!)
...
def analyze():
    ...
    try:
        ...
        logging.exception("❌ Error in /analyze: %s", e)
...
def contact():
    if request.method == 'POST':
        ...
        body = f"""
        📩 New message received from PancreScan Contact Form:
...
def custom_diet():
    ...
    if "user_id" not in session:
        return jsonify({"diet": ["⚠️ Please log in to view personalised diet."]})

    user = User.query.get(session["user_id"])

    if not user:
        return jsonify({"diet": ["⚠️ User not found."]})
...
def custom_exercise():
    ...
    # --- Age Based ---
    if age < 25:
        exercises.append("🏃‍♂️ 20–30 min jogging to boost overall fitness.")
        exercises.append("🚴 Cycling to improve stamina and metabolism.")
    elif 25 <= age < 40:
        exercises.append("🏋️ Strength training 3x/week for metabolic balance.")
        exercises.append("🧘 Yoga for flexibility & stress reduction.")
    elif 40 <= age < 60:
        exercises.append("🚶 Brisk walking for 30 minutes daily.")
        exercises.append("🧘‍♂️ Deep breathing + Pranayama for pancreas support.")
    else:
        exercises.append("🚶 Light walking 20 minutes/day.")
        exercises.append("🧘 Gentle stretching & chair yoga.")

    # --- Gender Based ---
    if gender.lower() == "male":
        exercises.append("💪 Upper-body strengthening with resistance bands.")
    elif gender.lower() == "female":
        exercises.append("🦵 Leg-strengthening & low-impact aerobic exercises.")
    else:
        exercises.append("✨ Balanced low-impact movements suitable for all.")

    # --- Cancer-based ---
    if cancer_detected:
        exercises.append("🧘‍♂️ Do soft yoga poses (Cobra, Child Pose, Cat-Cow).")
        exercises.append("🚶 Prefer gentle walking over intense cardio.")
    else:
        exercises.append("🏃 Maintain a healthy mix of cardio + stretching.")
...
def download_report():
    ...
    exercises = [
        "🏃 Brisk walking or cycling for 30 mins daily.",
        "🧘 Yoga poses like Bhujangasana and Vajrasana.",
        "🏋️ Light strength training under supervision."
    ]
    for line in exercises:
        pdf.drawString(80, y, line)
        y -= 18
        ...
        for col, x_pos in enumerate(x_positions):
            ...
            if image is not None:
                try:
                    ...
                    print(f"⚠️ Error drawing image {title}: {e}")
...
def chatbot():
    try:
        ...
        # ✅ Use the correct, available model from your list
...
def download_basic_report():
    try:
        ...
        # ✅ Get scan_id from form (each row sends its own scan_id)
        scan_id = request.form.get("scan_id")
        scan = ScanHistory.query.get(scan_id)

        if not scan:
            flash("⚠️ Scan not found.", "warning")
            return redirect(url_for('dashboard'))
        ...
        if cancerous:
            ...
            # Diet
            pdf.drawString(70, y, "✅ Recommended Diet:")
            y -= 15
            diet_points = [
            ...
            for item in diet_points:
                pdf.drawString(90, y, f"• {item}")
                y -= 15

            y -= 10
            pdf.drawString(70, y, "🚫 Foods to Avoid:")
            y -= 15
            avoid_points = [
            ...
            y -= 10
            pdf.drawString(70, y, "🏋️ Recommended Exercises:")
            y -= 15
            exercise_points = [
        ...
        pdf.drawString(60, 40, "Generated by Pancreatic Cancer Detection System – City Engineering College")
        ...
        logging.exception("❌ Error generating basic report: %s", e)
...
def reject_doctor(doctor_id):
    ...
    db.session.commit()
    flash("❌ Doctor rejected!", "danger")
    return redirect(url_for('admin_panel'))
@app.route('/doctor/user/<int:user_id>')
def doctor_view_user(user_id):
    if 'doctor_id' not in session:
        flash("⚠️ Please log in as doctor.", "warning")
        return redirect(url_for('doctor_login'))
...
def download_user_summary(user_id):
    ...
    pdf.drawString(60, y, f"👤 Name: {user.name}")
    y -= 20
    pdf.drawString(60, y, f"🎂 Age: {user.age}")
    y -= 20
    pdf.drawString(60, y, f"📞 Contact: {user.contact}")
    y -= 20
    pdf.drawString(60, y, f"📝 Total Scans: {total_scans}")
    y -= 20
    pdf.drawString(60, y, f"⚠️ Cancer Detected: {cancer_detected} time(s)")
    y -= 30
    ...
    # ==============================
    pdf.setFont("Helvetica-Bold", 14)
    pdf.drawString(60, y, "📊 Complete Scan History")
    y -= 20
    ...
    # FOOTER
    pdf.setFont("Helvetica-Oblique", 10)
    pdf.drawString(60, 40, "Generated by Doctor Dashboard – Pancreatic Cancer Detection System")
...
def mail_patient(user_id):
    ...
    doctor = Doctor.query.get(session['doctor_id'])   # ✅ FIX #1
...
"""
        ...
        flash("📧 Message successfully sent to patient!", "success")

    except Exception as e:
        print("Email Error:", e)
        flash("❌ Failed to send mail.", "danger")

    return redirect(url_for('doctor_view_user', user_id=user_id))
@app.route('/doctor/chatbot', methods=['POST'])
def doctor_chatbot():
    if "doctor_id" not in session:
        return jsonify({"reply": "⚠️ Access denied. Only doctors can use this chatbot."})
